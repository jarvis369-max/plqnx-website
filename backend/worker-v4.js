// PLQNX CORE 4: invite-only, D1-backed persistent conversations.
  ...
  async fetch(request,env) {
    ...
    try {
      if(path==="/v1/status"&&method==="GET")
        return reply({version:"persistent-v1",accounts:true,permanentMemory:true,securityVersion:"v4"},200,origin);
      if(path==="/v1/register"&&method==="POST"){
        ...
        if(typeof username==="string"){
          ...
          if(!await throttle(env.DB,env,"login-ip",ip,900,10) ||
             !await throttle(env.DB,env,"login-user",username.toLowerCase(),900,5))

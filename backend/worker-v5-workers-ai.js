...
// PLQNX 7.1 Cloudflare Workers AI adapter. Existing auth, D1 and chat API unchanged.
async function gemini(env,contents) {
  if(!env.AI)throw new Error("Workers AI binding AI is missing.");
  const messages=[
    {role:"system",content:"You are PLQNX CORE, a helpful AI assistant for learning and coding. Reply in Telugu when users write Telugu. Never claim PLQNX is incorporated or invent actions."},
    ...contents.map(turn=>({
      role:turn.role==="model"?"assistant":"user",
      content:(turn.parts||[]).filter(p=>typeof p.text==="string").map(p=>p.text).join("\n")
    }))
  ];
  let result;
  try{
    result=await env.AI.run("@cf/meta/llama-3.1-8b-instruct-fp8",{
      messages,max_tokens:450
    });
  }catch(error){
    console.warn("PLQNX Workers AI generation failed",error?.name||"Error");
  ...
  if(!answer)throw new Error("The AI service returned an empty reply.");
...
export default {
  async fetch(request,env) {
    ...
    if(!env.DB||!env.AI||!env.BETA_ACCESS_CODE)
      return reply({error:"Backend missing DB binding or secrets."},503,origin);
    let diagnosticStage="request";
    try {
      if(path==="/v1/status"&&method==="GET")
        return reply({version:"persistent-v1",accounts:true,permanentMemory:true,securityVersion:"v5.1",renameConversation:true,aiProvider:"cloudflare-workers-ai"},200,origin);

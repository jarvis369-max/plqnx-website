...
// Diagnostic-only fork of the 7.1 Worker. Do not log prompts, keys or responses.
// One upstream request per chat: avoids doubling usage while finding the cause.
// Optional, explicitly configured provider switch. Defaults to the original Gemini.
// To use Groq, deploy this file and set AI_PROVIDER=groq and GROQ_API_KEY as
// a Cloudflare SECRET. An AI_PROVIDER change affects future conversations only.
// Do not add the Groq key to GitHub or any public web page.
const AI_INSTRUCTIONS="You are PLQNX CORE, a helpful early-stage AI assistant for learning and coding. "+
  "Use Telugu when the user writes Telugu. Do not claim PLQNX is incorporated or invent actions.";
async function groqChat(env,contents){
  if(!env.GROQ_API_KEY)throw new Error("Groq is selected but GROQ_API_KEY is missing.");
  const model=env.GROQ_MODEL||"openai/gpt-oss-20b";
  if(!/^[a-zA-Z0-9._/-]{3,100}$/.test(model))throw new Error("Invalid Groq model configuration.");
  ...
  try{
    response=await fetch("https://api.groq.com/openai/v1/chat/completions",{
      ...
      body:JSON.stringify({
        ...
        messages:[
          ...
          ...contents.map(m=>({role:m.role==="model"?"assistant":"user",content:(m.parts||[]).map(p=>p.text||"").join("")}))
        ],
    ...
    console.warn("PLQNX_AI_DIAGNOSTIC",JSON.stringify({provider:"groq",phase:"fetch",model,
      durationMs:Date.now()-started,result:timeout?"TIMEOUT":"NETWORK_ERROR"}));
    const err=new Error(timeout?"Groq AI timed out. Try again later.":"Could not connect to Groq AI.");
    err.httpStatus=timeout?504:502;throw err;
  }
  if(!response.ok){
    console.warn("PLQNX_AI_DIAGNOSTIC",JSON.stringify({provider:"groq",phase:"response",model,
      durationMs:Date.now()-started,httpStatus:response.status}));
    const err=new Error(response.status===429?"Groq API rate limit reached. Try again later.":
      response.status===401||response.status===403?"Groq API key or model access denied.":
      response.status===404?"Configured Groq model not found.":
      "Groq API returned HTTP "+response.status+".");
    err.httpStatus=response.status===429?429:502;throw err;
  }
  const data=await response.json();
  const answer=typeof data?.choices?.[0]?.message?.content==="string"?data.choices[0].message.content.trim():"";
  console.info("PLQNX_AI_DIAGNOSTIC",JSON.stringify({provider:"groq",phase:"response",model,
...
async function geminiChat(env,contents){
  ...
  const body=JSON.stringify({systemInstruction:{parts:[{text:AI_INSTRUCTIONS}]},
  ...
  for(let attempt=1;attempt<=2;attempt++){
    ...
    try{
      response=await fetch(url,{method:"POST",headers:{"Content-Type":"application/json",
        "x-goog-api-key":env.GEMINI_API_KEY},body,signal:AbortSignal.timeout(remaining)});
    }catch(error){
      const timeout=error?.name==="TimeoutError"||error?.name==="AbortError";
      console.warn("PLQNX_AI_DIAGNOSTIC",JSON.stringify({provider:"gemini",phase:"fetch",model,
        turns:contents.length,chars:charCount,attempt,durationMs:Date.now()-started,
        result:timeout?"TIMEOUT":"NETWORK_ERROR"}));
      const err=new Error(timeout?"Gemini timed out.":"Gemini connection failed.");
      err.httpStatus=timeout?504:502;err.allowFallback=timeout;throw err;
    }
    if(!response.ok){
      let status="UNKNOWN";
      try{const errorBody=await response.json();const x=errorBody?.error?.status;
        if(typeof x==="string"&&/^[A-Z_]{2,50}$/.test(x))status=x}catch{}
      console.warn("PLQNX_AI_DIAGNOSTIC",JSON.stringify({provider:"gemini",phase:"response",model,
        turns:contents.length,chars:charCount,attempt,durationMs:Date.now()-started,
        httpStatus:response.status,upstreamStatus:status}));
      if(response.status===503&&attempt<2&&deadline-Date.now()>6000){
        await new Promise(resolve=>setTimeout(resolve,1000));continue;
      }
      const err=new Error(response.status===503?"Gemini is temporarily unavailable (HTTP 503).":
        response.status===429?"Gemini API quota limit reached.":response.status===404?
        "Gemini model not found.":"Gemini API returned HTTP "+response.status+".");
      err.httpStatus=response.status;err.allowFallback=response.status===503||response.status===504;
      throw err;
    }
    const data=await response.json();
    const answer=(data.candidates?.[0]?.content?.parts||[])
      .filter(p=>typeof p.text==="string").map(p=>p.text).join("").trim();
    console.info("PLQNX_AI_DIAGNOSTIC",JSON.stringify({provider:"gemini",phase:"response",model,
      turns:contents.length,chars:charCount,attempt,durationMs:Date.now()-started,httpStatus:200,hasText:!!answer}));
    if(!answer)throw new Error("Gemini returned no text.");
    return answer.slice(0,8000);
  }
  const err=new Error("Gemini timed out.");err.httpStatus=504;err.allowFallback=true;throw err;
}
async function gemini(env,contents){
  if(env.AI_PROVIDER==="groq")return groqChat(env,contents);
  try{return await geminiChat(env,contents)}
  catch(error){
    // Sending a conversation to Groq requires the owner's EXPLICIT configuration.
    // Do not silently route private chats to another provider.
    if(env.ENABLE_GROQ_FALLBACK==="true"&&env.GROQ_API_KEY&&error.allowFallback){
      console.warn("PLQNX_AI_DIAGNOSTIC",JSON.stringify({phase:"explicit_provider_fallback",from:"gemini",to:"groq"}));
      return groqChat(env,contents);
    }
    throw error;
  }
}
  ...
  async fetch(request,env) {
    ...
    try {
      if(path==="/v1/status"&&method==="GET")
        return reply({version:"persistent-v1",accounts:true,permanentMemory:true,securityVersion:"v5.1-multiprovider",renameConversation:true},200,origin);
      ...
      if(path==="/v1/chat"&&method==="POST"){
        ...
        try{answer=await gemini(env,contents)}
          ...
          // An unsuccessful request creates no conversation messages. Do not
          // charge the user's daily request allowance for it.
          try{await env.DB.prepare("UPDATE daily_usage SET requests=CASE WHEN requests>0 THEN requests-1 ELSE 0 END WHERE user_id=? AND day=?").bind(user.id,day).run()}catch{console.warn("PLQNX_AI_DIAGNOSTIC quota_refund_failed")}
          return reply({error:error.message||"AI unavailable."},error.httpStatus===503?503:error.httpStatus===429?429:502,origin);
        }
